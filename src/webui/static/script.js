...
async function switchPool(poolType) {
    try {
        const response = await fetch('/api/pool/switch', {
            method: 'POST',
            headers: {
        ...
        if (data.success) {
            ...
            document.querySelector(`[data-pool="${poolType}"]`).classList.add('active');
            
            // 更新池子信息
            updatePoolInfo(data.pool_info);
            
            // 重新加载配置和统计
            loadConfig();
            loadStatistics();
            
            console.log(`已切换到 ${poolType} 池`);
        }
    } catch (error) {
        console.error('切换池子失败:', error);
        alert('切换池子失败，请重试');
    }
}
...
async function loadPoolInfo() {
    try {
        ...
        if (data.success) {
            currentPoolType = data.pool_type;
            updatePoolInfo(data.pool_info);
        }
    } catch (error) {
...
function updatePoolInfo(poolInfo) {
    const descriptionEl = document.getElementById('poolDescription');
    if (descriptionEl) {
        descriptionEl.textContent = `${poolInfo.pool_name} - ${poolInfo.pool_description}`;
    }
}
